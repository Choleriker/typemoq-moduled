...
import { CtorWithArgs, IFunc2 } from './Common';
...
export class Mock<T> implements IMock<T> {
    ...
    static proxyFactory: IProxyFactory = new ProxyFactory();
    ...
    static ofType<U>(targetConstructor: CtorWithArgs<U>, behavior = MockBehavior.Loose, ...targetConstructorArgs: any[]): Mock<U> {
        let mock: Mock<U> = <any>Mock.ofType2(targetConstructor, targetConstructorArgs, behavior);
        return mock;
    }

    static ofType2<U>(targetConstructor: CtorWithArgs<U>, targetConstructorArgs: any[], behavior = MockBehavior.Loose): Mock<U> {
        let targetInstance: U = <any>Utils.conthunktor(targetConstructor, targetConstructorArgs);
